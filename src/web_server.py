...
Endpoints:
    ...
    GET  /api/sessions/grouped - Sessions grouped by project with aggregates
...
<head>
    ...
    <style>
        ...
        * {
        ...
        .event-row {
            display: grid;
            grid-template-columns: 80px 30px 100px 90px 120px 1fr;
            gap: 0.5rem;
            padding: 0.5rem;
            border-radius: 6px;
            align-items: center;
    ...
    <script>
        ...
        function updateEventsPanel() {
            ...
            panel.innerHTML = events.slice(0, 20).map(e => {
                ...
                const projectName = e.project || 'unknown';
                const displayAgent = formatAgentName(e.agent_name);
                
                return `
                    <div class="event-row">
                        <span class="time">${time}</span>
                        <span>${emoji}</span>
                        <span class="agent" style="color: ${color}">${displayAgent}</span>
                        <span class="project" title="${projectName}">${projectName}</span>
